...
class Client(object):
    ...
    def raw(self, stat, stats, sample_rate=1):
        """
        Log raw information for a single stat
        >>> statsd_client.raw('some.raw', 500)
        """
        stats = {stat: "%d|r" % stats}
        self.send(stats, sample_rate)

    def averaged(self, stat, stats, sample_rate=1):
        """
        Log averaged information for a single stat
        >>> statsd_client.averaged('some.averaged', 500)
        """
        stats = {stat: "%d|a" % stats}
        self.send(stats, sample_rate)
